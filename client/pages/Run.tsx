...
export default function Run() {
  ...
  const getCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        toast({
          title: "Lỗi GPS",
          description: "Không thể lấy vị trí hiện tại. Vui lòng cho phép truy cập vị trí.",
          variant: "destructive"
        });
      },
      {
  ...
  const getGpsStatusText = () => {
    switch (gpsStatus) {
      case "ready": return "GPS Sẵn sàng";
      case "searching": return "Đang tìm GPS...";
      case "error": return "GPS Lỗi";
    }
  };
      ...
      <div className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-vsm-black">Chạy bộ với GPS</h1>
              <p className="text-vsm-gray-medium">Theo dõi chính xác với định vị vệ tinh</p>
            </div>
                ...
                {!isRunning ? (
                  <div className="text-center">
                    ...
                    <h2 className="text-2xl font-bold text-vsm-black mb-2">
                      {gpsStatus === 'ready' ? 'Sẵn sàng chạy!' : 'Đang chuẩn bị GPS...'}
                    </h2>
                    ...
                    >
                      {gpsStatus === 'ready' ? 'Bắt đầu chạy' : 'Đang chờ GPS...'}
                    </Button>
                  </div>
                ) : (
                  <div className="text-center">
                    ...
                    {currentPosition && (
                      <div className="bg-gray-100 rounded-lg p-4 mb-6">
                        <p className="text-sm text-gray-600">
                          <strong>Vị trí hiện tại:</strong> {currentPosition.latitude.toFixed(6)}, {currentPosition.longitude.toFixed(6)}
                        </p>
                        <p className="text-sm text-gray-600">
                          <strong>Độ chính xác:</strong> {currentPosition.accuracy?.toFixed(0)}m
                        </p>
                      </div>
                    )}
              ...
              <CardContent>
                <div className="aspect-video bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg flex items-center justify-center">
                  <div className="text-center">
                    ...
                    {currentPosition ? (
                      <div>
                        <p className="text-gray-700 font-medium">Đang theo dõi GPS</p>
                        <p className="text-xs text-gray-500">
                          {currentPosition.latitude.toFixed(4)}, {currentPosition.longitude.toFixed(4)}
                        </p>
                      </div>
                    ) : (
                      <p className="text-gray-500">Bản đồ sẽ hiển thị khi có GPS</p>
                    )}
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold">{currentStats.distance}/5.0 km</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-vsm-orange h-2 rounded-full" 
                      style={{width: `${Math.min((parseFloat(currentStats.distance) / 5.0) * 100, 100)}%`}}
                    ></div>
                  </div>
                  
                  <div className="flex justify-between items-center">
                    <span className="text-sm">Calories</span>
                    <span className="font-semibold">{currentStats.calories}/400 calo</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-vsm-orange h-2 rounded-full" 
                      style={{width: `${Math.min((parseInt(currentStats.calories) / 400) * 100, 100)}%`}}
                    ></div>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="space-y-3">
                  {runHistory.slice(0, 5).map((run) => (
                    <div key={run.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      <div>
                        <div className="font-semibold text-sm">{run.distance} km</div>
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-gray-500">{run.pace}/km</div>
                      </div>
                    </div>
                  ))}
                  {runHistory.length === 0 && (
